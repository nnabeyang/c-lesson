...
static void test_stmdb() {
  struct AsmNode node;
  assert(asm_one("stmdb r13!, {r1}\n", &node, 0) != PARSE_FAIL);
  assert(node.type == WORD);
  assert(node.u.word == 0xE92D0002);
}

static void test_stmdb2() {
  struct AsmNode node;
  assert(asm_one("stmdb r13!, {r1, r2}\n", &node, 0) != PARSE_FAIL);
  assert(node.type == WORD);
  assert(node.u.word == 0xE92D0006);
}

static void test_stmdb3() {
  struct AsmNode node;
  assert(asm_one("stmdb r5!, {r1, r2, r0}\n", &node, 0) != PARSE_FAIL);
  assert(node.type == WORD);
